...
RSgen2.both.stream.allyears = glm.nb(noff.off~Origin.parents,data=d)
...
```{r}
d = subset(pedf,Year.off <= 2015 & Final_Status_ped.off=="Spawning Grounds" & Final_Status_ped.dam=="Broodstock" &  Final_Status_ped.sire=="Broodstock")
...
RSgen2.both.brood.allyears = glm.nb(noff.off~Origin.parents,data=d)
...
plot = ggplot(data=d,aes(y=Year.off,x=num,color=Origin_ped.off))  + geom_segment(xend = d$sirenum,yend = d$Year.sire,color="olivedrab",alpha=.1,linewidth=.05) + ylab("Year") + geom_segment(xend = d$damnum,yend = d$Year.dam,color="olivedrab",alpha=.1,linewidth=.05) +  geom_point(alpha=.6,size=.5,pch=16) + xlab("Spawners")  + ylab("Year") + scale_color_manual(values=c("indianred","lightsteelblue"))  + theme_cowplot() + theme(axis.text.x=element_blank()) + annotate("text",x=-6000,y=2018,label="Broodstock") + annotate("text",x=6000,y=2018,label="Spawning grounds") + theme(legend.position = "none") 
plot 
