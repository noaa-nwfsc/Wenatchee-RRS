...
d$num = as.integer(substr(d$id,6,9)) + 1000
d$sirenum = as.integer(substr(d$sire,6,9)) + 1000
d$damnum = as.integer(substr(d$dam,6,9)) + 1000
...
plot = ggplot(data=d,aes(y=Year.off,x=num,color=Origin_ped.off)) + geom_segment(xend = d$sirenum,yend = d$Year.sire,color="olivedrab",alpha=.1,linewidth=.05) + ylab("Year") + geom_segment(xend = d$damnum,yend = d$Year.dam,color="olivedrab",alpha=.1,linewidth=.05) +  geom_point(alpha=.6,size=1,pch=16) + xlab("Spawners")  + ylab("Year") + scale_color_manual(values=c("indianred","lightsteelblue"))
plot
