...
plot = ggplot(data=d,aes(y=Year.off,x=Dayofyear.off,color=Origin_ped.off)) + geom_segment(xend = d$Dayofyear.sire,yend = d$Year.sire,color="olivedrab",alpha=.1,linewidth=.05) + ylab("Year") + geom_segment(xend = d$Dayofyear.dam,yend = d$Year.dam,color="olivedrab",alpha=.1,linewidth=.05) +  geom_point(alpha=.2,size=1,pch=16) + xlab("Day of year")  + ylab("Year") + xlab("Day of year") + scale_color_manual(values=c("indianred","lightsteelblue"))
plot
...
Try using the id numbers instead of the day of year - not as nice

```{r}
d = subset(pedf,Year.off <=2018)
d$idnum = as.integer(substr(d$id,6,9))
d$damnum = as.integer(substr(d$dam,6,9))
d$sirenum = as.integer(substr(d$sire,6,9))

d$idnum = ifelse(d$Final_Status_ped.off=="Broodstock",-1*d$idnum,d$idnum)
...
ggplot(data=d,aes(y=Year.off,x=idnum,color=Origin_ped.off)) + geom_point(alpha=.8,size=.5) + scale_color_manual(values = c("red","blue")) + geom_segment(xend = d$sirenum,yend = d$Year.sire,color="grey",alpha=.05) + ylab("Year") + xlab("Day of year")
