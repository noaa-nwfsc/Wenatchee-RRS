...
Try using the ordered numbers instead of the day of year
...
d$num = substr(d$id,6,9)
d$sirenum = substr(d$sire,6,9)
d$damnum = substr(d$dam,6,9)
...
plot = ggplot(data=d,aes(y=Year.off,x=num,color=Origin_ped.off)) + geom_segment(xend = d$sirenum,yend = d$Year.sire,color="olivedrab",alpha=.1,linewidth=.05) + ylab("Year") + geom_segment(xend = d$damnum,yend = d$Year.dam,color="olivedrab",alpha=.1,linewidth=.05) +  geom_point(alpha=.2,size=1,pch=16) + xlab("Day of year")  + ylab("Year") + xlab("Day of year") + scale_color_manual(values=c("indianred","lightsteelblue"))
plot

pdf("Wenatchee_pedigree_figure_idnums.pdf",width=10,height=8)
plot
dev.off()
